...
void Draw::renderRect ()
{
  ...
  if (pointlist.size () / 4 != (colorlist.size ()))
    std::
      cout <<
      "Error,mismatch between number of colors and points in the rectangle" <<
      std::endl;
...
void drawWall (std::vector < bool > wallx, int wallwidth, int wallheight,
	  Draw * drawobj, char *color, double xscale, double yscale,
	  bool isvert)
{
  int x;
  int y;
  ...
  while (!wallx.empty ())
      ...
      if (row <= 0)
	{
      ...
      if (wallx.back () == 1)
	{
	  drawobj->pushRect (color, x, y,
			     pixwidth * xscale +
			     (pixwidth * (!isvert) * yscale),
			     pixwidth * yscale +
			     (pixwidth * isvert * xscale));
	}
...
public:
  ...
  struct walls
  {
    std::vector < bool > horwalls, vertwalls;
    int width;
    int height;
  } wall;
  enum direction
...
bool
Maze::checkVisited (direction dir)
{
  int xcheck = 0;
  int ycheck = 0;
...
void
Maze::startCreation (bool xloops)
{
  srand (time (NULL));
...
void
Maze::moveForward (direction dir)
{
  if (dir == left)
    xpos--;
  if (dir == right)
...
Maze::checkAnyVis ()
{
  ...
  if (checkVisited (left) == 1 && checkVisited (right) == 1
      && checkVisited (up) == 1 && checkVisited (down) == 1)
    {
      return 1;
    }
  else
...
void
drawBlock (int xp, int yp, Draw * drawobj, Maze * maze, char *color)
{
...
void
Maze::drawValues (Draw * xwin, int ra, int ga, int ba, int rb, int gb, int bb)
{
...
//1 means it loops
  ...
  if (wallstrue != 0)
    {
      //horizontal
      drawWall (maze.wall.horwalls, maze.wall.width, maze.wall.height, &xwin,
		black, 1.0, 1.0 / 2.0, 0);
      //verticle
      drawWall (maze.wall.vertwalls, maze.wall.width + 1,
		maze.wall.height - 1, &xwin, black, 1.0 / 2.0, 1.0, 1);
  ...
  double distance =
    sqrt (pow ((colors[0] - colors[3]), 2) +
	  pow ((colors[1] - colors[4]), 2) + pow ((colors[3] - colors[5]),
						  2));
  ...
  maze.drawValues (&xwin, colors[0], colors[1], colors[2], colors[3],
		   colors[4], colors[5]);
